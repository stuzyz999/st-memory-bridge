...
- sillytavern LLM API配置（模型拉去、多配置持久化）、正文正则清洗、上下文读取（历史正文、世界书、user设定）、内容选择、半自动摄入（楼层设置）、多tab交互、提示词系统等可复用参考项目的方案，一律移植，最小化代码，最快落地。
- mcp召回优先分析nocturne_memory数据特点，保留细节、入库效率和召回准确率。
- 参考目标项目做分段提示词，提示词角色定义直接照搬，数据处理核心做独立特化
- 移植为主，适配为辅
- 不开发独立算法，只做移植整合。信息来源项目路径内参考目标>搜索工具github查询
- 沟通汇报请使用简体中文。
